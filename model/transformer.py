...
def multi_head_attention_forward(
    ...
    if torch.isnan(attn_output_weights).any():
        attn_output_weights = torch.where(torch.isnan(attn_output_weights),torch.full_like(attn_output_weights,0),attn_output_weights)
...
class MultiheadAttention(Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None,
                need_weights=False, need_raw=False, attn_mask=None):
...
class FeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout = 0., last_layer=False):
        super().__init__()
        if not last_layer:
    ...
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout = 0.):
        ...
        for _ in range(depth):
            if _ < depth-1:
                ...
                self.layers.append(nn.ModuleList([
                    Attention(dim, heads = heads, dim_head = dim_head, dropout = dropout),
                    FeedForward(dim, mlp_dim, dropout = dropout, last_layer=True)
                ]))
